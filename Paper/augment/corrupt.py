...
* **Oracle-blind positives** (``HP_oracle_blind``): genuine rhymes the 5.3.5
  ...
  here for missing them (its documented limitation), while checkers that
  hear the real rhyme get the credit. Each is verified blind on the actual
  wak (same ssg as the checkers) and standalone; the review file records the
  mechanism (silent_r / first_sara) per row.
...
# Thai-script pronunciation for the human-readable review columns -- NEVER
# used for the gold or the checkers. Prefers the curated POETRY_OVERRIDES
# (authoritative); otherwise w2p. IMPORTANT: w2p is a learned Word-to-Phoneme
# model and CAN hallucinate, especially on short / multi-syllable words, so
# every row records the pronunciation SOURCE and a reviewer must treat w2p
# as a hint to cross-check (IPA / syllable decomposition), never as truth.
_PRON_CACHE: dict = {}


def _pron(w: str):
    """Return (thai-script pronunciation, source) for ``w``. Source is
    'override' (curated POETRY_OVERRIDES, authoritative), 'w2p' (ML model -
    may hallucinate, cross-check!), or 'none' (falls back to ``w`` itself).
    """
    hit = _PRON_CACHE.get(w)
    if hit is not None:
        return hit
    ovr = POETRY_OVERRIDES.get(w)
    if ovr:
        hit = ("".join(ovr), "override")
    else:
        try:
            p = pronunciate(w, engine="w2p")
        except (ValueError, TypeError, KeyError, IndexError, OSError,
                RuntimeError, AttributeError, ModuleNotFoundError):
            p = ""
        if p:
            hit = (p.replace("ฺ", "").replace("-", ""), "w2p")
        else:
            hit = (w, "none")
    _PRON_CACHE[w] = hit
    return hit
...
REVIEW_HEADER = [
    ...
    "S_pron", "S_pron_src", "C_pron", "C_pron_src",
    "partner",
    "tag", "both_norm", "oldA_fail", "broken",
    "loc", "note",
]

# Review-file usage note (also written to Paper/augment/output/README.md).
REVIEW_CAUTION = (
    "pron columns = Thai-script pronunciation for cross-checking only.\n"
    "  source 'override' = curated POETRY_OVERRIDES (authoritative);\n"
    "  source 'w2p' = pythainlp w2p ML model -- CAN hallucinate, especially "
    "on short / multi-syllable words; treat as a HINT, verify with IPA and "
    "syllable decomposition, never trust it as ground truth.\n"
    "partner = the rhyme-target syllables of the rule (avoid set), each with "
    "its (สระ/มาตรา) as read by the 5.3.5 oracle -- the side C must rhyme "
    "with (positives) or must NOT rhyme with (negatives)."
)
...
def main() -> None:
    ...
    ap.add_argument("--ob-positives", type=int, default=250,
                    help="target oracle-blind positives per rule (default "
                         "250; limited by the blind-family target "
                         "availability in the corpus)")
    ...
    def _emit(kind, s, rid, waks, prev_w4, S, C, op, broken, wak_idx,
        ...
        orig_waks = s.waks()
        _, _, avoid = _rule_site(rid, s)
        partner_str = ";".join(
            f"{t}({kv.check_sara(t)}/{kv.check_marttra(t)})" for t in avoid
        ) or "-"
        s_pron, s_psrc = _pron(S)
        c_pron, c_psrc = _pron(C)
        row = [
    ...
    # false negatives here for missing them). Families: silent-ร -> เอะ+กด
    # (เพชร/เนตร/เกษตร/เขตร) and first_sara -> อู+กา / อี+กา (ศัตรู/กษัตรี/
    # กษัตรีย์). Every emitted instance is verified blind on the ACTUAL wak
    # (same ssg as the checkers) and standalone. ----
    ...
    for rid in RULES:
        for story in stories:
            ...
            for s in by_story[story]:
                ...
                if not cands:
                    continue
                rng.shuffle(cands)  # rotate through all blind words
                for C in cands:
                    ...
                    _emit("positive", s, rid, waks, s.prev_w4, S, C,
                          ...
                          tag="oracle_blind",
                          note=f"oracle-blind "
                               f"({ORACLE_BLIND_REASON.get(C, '?')}): "
                               "real rhyme the oracle misses")
    ...
    print(f"  by candidate: {dict(ob_by_cand.most_common())}", flush=True)
    ...
    with open(os.path.join(args.out_dir, "README.md"),
              "w", encoding="utf-8") as f:
        f.write("# Augmentation review files\n\n")
        f.write("Generated by `Paper/augment/corrupt.py` "
                f"(seed {args.seed}, {len(instances)} instances).\n\n")
        f.write("- `review_negatives.tsv` -- precision probes (gold=0).\n")
        f.write("- `review_positives.tsv` -- recall probes, incl. "
                "oracle-blind (gold=1).\n")
        f.write("- `candidates_review.tsv` -- candidate word list per "
                "rhyme class.\n\n")
        f.write("## Column usage note\n\n")
        f.write("```\n" + REVIEW_CAUTION + "\n```\n")
